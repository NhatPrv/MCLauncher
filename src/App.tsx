...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN COMPONENT — FLUID RESPONSIVE GRID LAYOUT
═══════════════════════════════════════════════════════════════════════════ */
export function App() {
  ...
  return (
    <div className={`w-screen h-screen flex flex-col overflow-hidden select-none ${dark ? "dark bg-slate-950 text-slate-100" : "bg-slate-100 text-slate-900"}`}>
      {/* Background patterns */}
      {dark && (
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 flex-shrink-0 h-14 border-b backdrop-blur-md transition-colors" style={{ borderColor: border, background: dark ? "rgba(15,23,42,0.88)" : "rgba(255,255,255,0.88)" }}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {([
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-full text-xs font-semibold transition-all duration-200"
              style={{
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setIsSettingsOpen(true)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:scale-105" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(226,232,240,0.7)", color: sub }}>
            <Settings size={14} />
          </button>
          <button onClick={handleToggleTheme} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg transition-all hover:scale-105" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(226,232,240,0.7)", color: sub }}>
      ...
      {/* ═══ MAIN CONTENT (Fluid Flex Scrollable Area) ═══ */}
      <main className="flex-1 overflow-y-auto relative z-10 px-4 md:px-8 py-5 space-y-5" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(16,185,129,0.2) transparent" }}>

        {/* ─── TAB: HOME ─── */}
        {activeTab === "home" && (
          <div className="max-w-7xl mx-auto space-y-5">
            {/* Hero Banner Responsive Grid Split */}
            <div className="relative rounded-2xl overflow-hidden min-h-[200px] flex flex-col md:flex-row" style={{ background: dark ? "#0c1322" : "#e2e8f0", border: `1px solid ${border}` }}>
              {/* Left Content Area */}
              <div className="flex-1 p-6 z-10 flex flex-col justify-center max-w-2xl">
                ...
                <h1 className="text-2xl md:text-3xl font-extrabold leading-tight tracking-tight text-white mb-2">
                ...
                <div className="flex items-center gap-2 mb-4 text-xs">
                  ...
                  <span className="text-slate-400 truncate max-w-sm transition-all duration-300" key={noteIdx}>
                    {PATCH_NOTES[noteIdx]}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-300 hover:text-white transition-all" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(226,232,240,0.7)", border: `1px solid ${border}` }}>
              ...
              <div className="relative md:w-1/2 min-h-[160px] md:min-h-full overflow-hidden">
                <img src="https://images.unsplash.com/photo-1629429407759-01cd3d7cfb38?w=800&h=400&fit=crop&auto=format" alt="Minecraft Artwork" className="w-full h-full object-cover" style={{ filter: "brightness(0.65) saturate(1.2)" }} />
                <div className="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-r from-[#0c1322] via-transparent to-transparent" />
                <div className="absolute top-3 right-4 flex items-center gap-1 text-[10px] text-emerald-400 font-semibold z-10 bg-slate-950/60 px-2.5 py-1 rounded-full backdrop-blur-sm border border-emerald-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {/* News Card */}
              <div className="rounded-2xl overflow-hidden transition-all hover:scale-[1.01] cursor-pointer group" style={{ background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(8px)" }}>
                ...
                <div className="p-4">
                  <h3 className="font-bold text-sm mb-1.5 leading-snug">Mob Vote 2024: Armadillo Wins Community Poll</h3>
                  <p className="text-xs text-slate-400 leading-relaxed mb-3">The Armadillo claimed 42% of votes. Now live in Minecraft 1.21 with Wolf Armor crafting.</p>
                  <span className="text-xs text-emerald-400 font-semibold flex items-center gap-1 group-hover:gap-2 transition-all">Read more <ArrowRight size={11} /></span>
                </div>
              </div>
              ...
              <div className="rounded-2xl overflow-hidden transition-all hover:scale-[1.01] cursor-pointer group" style={{ background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(8px)" }}>
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs text-slate-500 font-mono">mc1.21.1</span>
                  </div>
                  <p className="text-xs text-slate-400 leading-relaxed mb-3">Improved 1.21.1 support, 18% faster mod loading, and 6 critical bug fixes.</p>
              ...
              {/* Iris Shaders Card */}
              <div className="rounded-2xl overflow-hidden transition-all hover:scale-[1.01] cursor-pointer group md:col-span-2 lg:col-span-1" style={{ background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(8px)" }}>
                <div className="relative h-32 overflow-hidden">
                  ...
                  <div className="absolute top-2.5 right-2.5 flex items-center gap-0.5 px-2 py-0.5 rounded bg-black/50 text-amber-400 text-xs font-bold backdrop-blur-sm">
                    <Star size={10} fill="currentColor" /> 4.9
                  </div>
                  <div className="absolute bottom-2 left-3 text-xs font-bold text-cyan-400 tracking-wider uppercase">✦ Iris Shaders</div>
                </div>
                <div className="p-4">
                  <h3 className="font-bold text-sm mb-1.5">Complementary v5.3</h3>
                  <p className="text-xs text-slate-400 leading-relaxed mb-3">Volumetric fog, PBR lighting, ray-traced reflections — runs at 90fps on RTX 4070.</p>
                  <span className="text-xs text-emerald-400 font-semibold flex items-center gap-1"><Download size={11} /> Get Shaders Pack</span>
                </div>
              </div>
            </div>

            {/* Featured Servers Row */}
            <div className="rounded-2xl p-4" style={{ background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(8px)" }}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Server size={14} className="text-slate-400" />
                  <span className="font-bold text-xs uppercase tracking-wider text-slate-300">Featured Servers</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-semibold">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> 3 / 3 online
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {SERVERS.map((s) => (
                  <div key={s.name} className="flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all hover:scale-[1.01]" style={{ background: dark ? "rgba(15,23,42,0.5)" : "rgba(248,250,252,0.8)", border: `1px solid ${dark ? "rgba(30,41,59,0.6)" : "#e2e8f0"}` }}>
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0" style={{ background: dark ? "rgba(30,41,59,0.8)" : "#f1f5f9" }}>{s.icon}</div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] text-slate-500 font-mono">{s.address}</span>
                        <span className="text-[10px] text-slate-400 flex items-center gap-1"><Users size={9} />{s.players.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {activeTab === "mods" && (
          <div className="max-w-7xl mx-auto space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <h2 className="text-xl font-bold">Mods & Modpacks Gallery</h2>
              <div className="relative w-full sm:w-64">
                <Search size={14} className="absolute left-3 top-2.5 text-slate-400" />
                <input type="text" placeholder="Search mods..." className="w-full pl-9 pr-3 py-2 rounded-xl text-xs font-semibold outline-none text-slate-100" style={{ background: dark ? "rgba(15,23,42,0.6)" : "#fff", border: `1px solid ${border}` }} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {MODS.map((m) => (
                <div key={m.name} className="p-4 rounded-xl flex items-center justify-between" style={{ background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(8px)" }}>
                  <div>
                    <div className="flex items-center gap-2">
        ...
        {activeTab === "account" && (
          <div className="max-w-2xl mx-auto space-y-5">
            ...
            {account && (
              <div className="p-5 rounded-2xl flex items-center justify-between shadow-lg" style={{ background: cardBg, border: `1px solid ${border}` }}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-bold text-base">{account.username}</div>
                    <div className="text-xs text-emerald-400 font-semibold flex items-center gap-1.5 mt-0.5">
                      <span className="w-2 h-2 rounded-full bg-emerald-400" /> {account.account_type} Mode
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="p-5 rounded-2xl space-y-3 shadow-lg" style={{ background: cardBg, border: `1px solid ${border}` }}>
              ...
              <form onSubmit={handleSaveOfflineName} className="flex gap-2">
                <input type="text" value={offlineNameInput} onChange={(e) => setOfflineNameInput(e.target.value)} className="flex-1 p-2.5 rounded-xl text-xs font-mono text-slate-100 outline-none focus:border-emerald-500" style={{ background: dark ? "rgba(15,23,42,0.7)" : "#f8fafc", border: `1px solid ${border}` }} />
      ...
      {/* ═══ BOTTOM ACTION BAR (Fluid Responsive Footer) ═══ */}
      <footer className="relative z-20 flex flex-wrap items-center justify-between px-6 py-3 gap-4 flex-shrink-0 border-t backdrop-blur-xl transition-colors" style={{ borderColor: border, background: dark ? "rgba(15,23,42,0.92)" : "rgba(255,255,255,0.92)" }}>
        ...
        <div onClick={() => setActiveTab("account")} className="flex items-center gap-3 px-3.5 py-2 rounded-xl cursor-pointer transition-all hover:scale-[1.01]" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(248,250,252,0.9)", border: `1px solid ${border}`, minWidth: 170 }}>
        ...
        <div className="flex items-center gap-2 flex-1 max-w-md" ref={dropRef}>
          <div className="relative flex-1">
            <button onClick={() => setVersionOpen(!versionOpen)} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs transition-all hover:scale-[1.01]" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(248,250,252,0.9)", border: `1px solid ${border}` }}>
              <LoaderBadge loader={selectedVersion.loader} />
              <div className="flex-1 text-left">
                <div className="font-bold text-xs">{selectedVersion.label}</div>
            ...
            {versionOpen && (
              <div className="absolute bottom-full mb-2 left-0 right-0 rounded-xl overflow-hidden p-1 shadow-2xl z-50 backdrop-blur-xl" style={{ background: dark ? "#18181b" : "#fff", border: `1px solid ${border}` }}>
                {VERSIONS.map((v) => (
                  <button key={v.id} onClick={() => { setSelectedVersion(v); setVersionOpen(false); }} className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs text-left transition-colors ${selectedVersion.id === v.id ? "bg-emerald-500/15 text-emerald-400 font-bold" : "hover:bg-slate-800/50 text-slate-300"}`}>
          ...
          <button className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:scale-105" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(248,250,252,0.9)", border: `1px solid ${border}`, color: sub }}>
            <RefreshCw size={14} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <button onClick={handleOpenFolder} className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:scale-105" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(248,250,252,0.9)", border: `1px solid ${border}`, color: sub }} title="Open .minecraft">
            <Folder size={14} />
          </button>
          <button onClick={() => setIsSettingsOpen(true)} className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:scale-105" style={{ background: dark ? "rgba(30,41,59,0.6)" : "rgba(248,250,252,0.9)", border: `1px solid ${border}`, color: sub }} title="Settings">
            <Settings size={14} />
          </button>

          {/* Glowing PLAY Button */}
          <button disabled={isLaunching} onClick={handlePlay} className="flex items-center gap-3 px-7 rounded-xl font-bold transition-all duration-150 hover:scale-[1.03] active:scale-95" style={{ height: 46, background: "linear-gradient(135deg,#10b981 0%,#059669 50%,#047857 100%)", color: "#fff", boxShadow: "0 0 28px rgba(16,185,129,0.5), 0 4px 14px rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.15)" }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
          <div className="w-full max-w-md rounded-2xl shadow-2xl p-5 space-y-4" style={{ background: dark ? "#0f172a" : "#fff", border: `1px solid ${border}`, color: dark ? "#e2e8f0" : "#0f172a" }}>
            ...
            <div className="flex justify-end gap-2 pt-2" style={{ borderTop: `1px solid ${border}` }}>
              <button onClick={() => setIsSettingsOpen(false)} className="px-3.5 py-1.5 rounded-xl text-xs font-bold" style={{ background: dark ? "rgba(30,41,59,0.6)" : "#e2e8f0", color: sub }}>Cancel</button>
              <button onClick={() => { invoke("update_config", { config }).catch(() => {}); setIsSettingsOpen(false); }} className="px-4 py-1.5 rounded-xl bg-emerald-600 text-xs font-bold text-white shadow-md">Save</button>
            </div>
          </div>
        </div>
      )}
